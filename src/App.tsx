    ...
    <>
      ...
      <div className="text-white p-3">
        <nav className="flex flex-row gap-2 w-1/2 justify-between m-auto absolute top-1/2 left-1/4 font-extrabold text-xl">
          ...
          }}><a className="hover:text-yellow-400 flex flex-col items-center" href="https://youtube.com"><FaYoutube className="text-red-600"/> youtube</a></div>
          <div style={{
            color: "rgba(220, 210, 210, 0.82)"
        ...
        <div className="latest-news pt-4 pl-10 absolute top-full left-0 right-0 bg-neutral-900">
          ...
          <div className="articles container grid grid-cols-2 gap-2 w-11/12 overflow-y">
            {news && news.results.map((article) => (<Article article={article} key={article.uri}/>))}
          </div>
        </div>
      </div>
    </>
